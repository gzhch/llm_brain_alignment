...
log_file = open('/ossfs/workspace/nas/gzhch/br/ACL_result/layerwise_proj_ppl.jsonl', 'w')

random_net = LinearProjector(11008, 11008).half()
zero_net = LinearProjector(11008, 11008).half()
zero_net.fc.weight.data = torch.zeros(zero_net.fc.weight.data.shape)
zero_net.fc.bias.data = torch.zeros(zero_net.fc.bias.data.shape)
...
# for layer1 in [0]:
    ...
    # for layer2 in [10]:
        ...
        ppl = evaluate_ppl(eval_data, model, proj)
        ppl_random = evaluate_ppl(eval_data, model, random_proj)
        ppl_zero = evaluate_ppl(eval_data, model, random_proj)

        json.dump(dict(layer1=layer1, layer2=layer2, ppl=ppl, ppl_random=ppl_random), log_file)
        log_file.write('\n')
        log_file.flush()
